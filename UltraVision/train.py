...
def train_simclr(model, optimizer, criterion, data_loader, num_epochs,
                 writer, save_model_dir, temperature=0.1):

    def info_nce_loss(features):

        labels = torch.cat([torch.arange(data_loader.batch_size) for i in range(2)], dim=0)
    ...
    for epoch_counter in pbar:
        ...
        if writer:
            writer.add_scalar('loss/simclr', running_loss / len(data_loader), epoch_counter)
        if epoch_counter % 100 == 0 or epoch_counter == num_epochs - 1:
            custom_models.save_model(model, os.path.join(save_model_dir, f"{epoch_counter}.pt"))
