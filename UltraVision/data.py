...
def FetalPlanes_numpy(csv_f,
    ...
    return imagenames, imagelocs, imagearrays, planesout, X_train, X_test,  X_val, y_train, y_test, y_val
...
class FetalPlanesTransform:
    ...
    """
    def __init__(self, resize=(300, 400),
                       crop=(224, 224),
                       one_channel=True,
        ...
        self.train = transforms.Compose([
            ...
            transforms.ToTensor(),
            transforms.Normalize(*normalize) if normalize_option else transforms.Lambda(lambda x: x)
        ]
        )
        self.test = transforms.Compose([
            transforms.ToPILImage(),
...
def FetalPlanes(directorycsv, directoryimg, val_size, batch_size, one_channel, use_og_split=True,
                normalize_option="paper", split_seed=10):
    _, _, _, _, X_train, X_test,  X_val, y_train, y_test, y_val = FetalPlanes_numpy(directorycsv, directoryimg, val_size,
                                                                                    use_og_split=use_og_split)
    
    transform = FetalPlanesTransform(one_channel=one_channel, normalize_option=normalize_option)
...
def FetalPlanes2(label_dir, data_dir, validation_split, batch_size, split_seed=10,
                    normalize_option="paper", problem_type="MultiClass_FP",
                                        one_channel=False):
    """
    ...
    transforms = FetalPlanesTransform(one_channel=one_channel, normalize_option=normalize_option)
