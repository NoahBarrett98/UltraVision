...
@click.option("--data_name", default="", help="name of dataset")
@click.option(
    "--test_size", default=0.1, type=float, help="percentage to be used for test set"
)
...
def runSciLearn(label_dir, data_dir, data_name, test_size, resizex, resizey):
    ...
    (
        ...
        Y_test,
        Y_val,
    ) = data.FetalPlanes_numpy(label_dir, data_dir, test_size)
    ...
    for c in np.logspace(-1, 2, num=13):
        ...
        hogSVCClassReport = pd.DataFrame(
            models.PredictAndClass(hogSVC, hogX_train, Y_train)
        )
